...
const RegisterScreenTeacher = ({ navigation }) => {
    ...
    const procesar = () => {
        ...
        if (nomApe === '') {
            ...
            setError('email')
            setErrorMsj('El email no es válido')
            return
        }
        else if (username === '') {
            setError('username')
    ...
    const validar = async () => {
        ...
        try {
            const response = await fetch(url + 'api/registerTeacher', {
                method: 'POST',
                headers: {
            ...
            if (result.success === true) {
                createTeacher(result.id);
                navigation.navigate('RegisteredUser', { nomApe: nomApe, rol: 'teacher' });
            }
    ...
    return (
        <ImageBackground source={require('../assets/images/fondo1.png')} resizeMode='cover' style={styles.image}>
            <ScrollView>
                ...
                />
                {error === 'email' ? <Text style={{ color: 'red', textAlign: 'center', marginTop: -10, marginBottom: 5 }}>{errorMsj}</Text> : null}
                <InputWithImage
                    imageSource={require('../assets/images/user_icon.png')} // Reemplaza con la ruta de tu imagen
                    placeholder="Usuario"
                    value={username}
...
export default RegisterScreenTeacher;